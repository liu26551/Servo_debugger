#ifndef LOBOTBUSSERVO_HPP
#define LOBOTBUSSERVO_HPP
...
private:
    ...
    /* data */
public:
    LobotBusServo(HardwareSerial &A);
...
/// @brief
LobotBusServo::LobotBusServo(HardwareSerial &A)
{
    SerialX = (Stream *)(&A);
}

LobotBusServo::~LobotBusServo()
{
}

// 校验和
byte LobotBusServo::LobotCheckSum(byte buf[])
{
    byte i;
    uint16_t temp = 0;
    for (i = 2; i < buf[3] + 2; i++)
...
// 解析接收到的数据包信息，并返回
int LobotBusServo::ReceiveHandle(byte *ret)
{
...
// 写入舵机ID
void LobotBusServo::SetID(uint8_t oldID, uint8_t newID)
{
    byte buf[7];
...
// 控制舵机转动
void LobotBusServo::Move(uint8_t id, int16_t position, uint16_t time)
{
    byte buf[10];
    if (position < 0)
        position = 0;
...
// 读取舵机位置
int LobotBusServo::ReadPosition(uint8_t id)
{
    int count = 10000;
    int ret;
    byte buf[6];
...
// 读取偏差
int LobotBusServo::ReadDev(uint8_t id)
{
    int count = 10000;
    int ret;
    byte buf[6];
...
int LobotBusServo::ReadAngleRange(uint8_t id)
{
    int count = 10000;
    int ret;
    byte buf[6];
...
// 读取电压
int LobotBusServo::ReadVin(uint8_t id)
{
    int count = 10000;
    int ret;
    byte buf[6];
...
// 读取电压范围
int LobotBusServo::ReadVinLimit(uint8_t id)
{
    int count = 10000;
    int ret;
    byte buf[6];
...
// 读取温度报警阈值
int LobotBusServo::ReadTempLimit(uint8_t id)
{
    int count = 10000;
    int ret;
    byte buf[6];
 ...
 * @return int 
 */
int LobotBusServo::ReadTemp(uint8_t id)
{
    int count = 10000;
    int ret;
    byte buf[6];
 ...
 * @return int 
 */
int LobotBusServo::ReadLoadOrUnload(uint8_t id)
{
    int count = 10000;
    int ret;
    byte buf[6];
 ...
 * @param id 
 */
void LobotBusServo::StopMove(uint8_t id)
{
    byte buf[6];
    buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
    buf[2] = id;
 ...
 * @param Speed 
 */
void LobotBusServo::SetMode(uint8_t id, uint8_t Mode, int16_t Speed)
{
    byte buf[10];
...
// 舵机上电
void LobotBusServo::Load(uint8_t id)
{
    byte buf[7];
    buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
    buf[2] = id;
...
// 舵机掉电
void LobotBusServo::Unload(uint8_t id)
{
    byte buf[7];
    buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
    buf[2] = id;
